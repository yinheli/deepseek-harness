...
当 `ctx.fs.sandboxMode` 表明提供方施加沙箱限制时，write/edit 会公开 `sandbox_permissions` 与 `justification`，并通过 `ctx.approval` 处理获批后的重试。由于 schema 属于注册表全局定义，而有效模式按会话确定，目标等于当前模式时会作为冗余参数接受，无需理由或审批；只有真正加宽时才校验配对理由并发起询问。更窄或未知目标仍会在变更前失败。策略归属方会贡献与具体能力无关的常驻策略；工具结果仍保留针对具体操作的拒绝与重试引导。
