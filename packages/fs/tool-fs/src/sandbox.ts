...
import { ESCALATION_TARGETS, approveEscalation, escalationHintMarker, sandboxDenialMarker, sandboxPermissionsDescription } from '@deepseek-ai/dsh-sandbox'
...
export class FsSandboxController {
  ...
  /**
   * Resolve this mutation's policy. Redundant same-mode arguments preserve the
   * standing policy without approval; genuine widening is validated and
   * approved by the shared escalation owner. The calling session's cwd is
   * always carried as the workspace root.
   * @param toolName - the mutating tool's name, for the approval audit trail.
   * @param args - the call's optional escalation arguments.
