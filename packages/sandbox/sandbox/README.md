...
The ladder is a closed table — `read-only` may escalate to `workspace-write` or `danger-full-access`, `workspace-write` only to `danger-full-access` — checked at execution, never baked into a tool schema, whose enum stays the closed target vocabulary. [`approveEscalation`](src/escalation.ts) short-circuits a request naming the call's own mode, validates the `sandbox_permissions`/`justification` pairing for every other request, rejects narrower or unknown targets without prompting a human, and maps every approval outcome to its own error before anything executes.
