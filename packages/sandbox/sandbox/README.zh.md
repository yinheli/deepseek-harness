...
受限调用被拒绝时，操作会报告指明模式的拒绝标记——`[sandbox: file access denied under <mode> mode]`——组合声明升权能力时还会给出升权提示。模型可以用 `sandbox_permissions`（足以放行的最窄更宽模式）加 `justification` 重试一次完全相同的调用；用户会看到一次审批提示，可以选择允许一次、拒绝或取消。真正的升权必须严格宽于调用的生效模式，且只作用于该次调用。指定调用自身模式的请求属于冗余元数据：无需理由与提示即可接受；更窄或不在阶梯内的目标一律失败关闭。
